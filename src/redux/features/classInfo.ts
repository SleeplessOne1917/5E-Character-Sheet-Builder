...
export const reducers: AppReducers<ClassInfoState> = {
	...
	setAbilityBonus: (
		state,
		{ payload: { abilityScores, index } }: PayloadAction<SetAbilityBonusPayload>
	) => {
	...
	removeFeatureSubfeature: (
		...
		state.featuresSubfeatures[index] = state.featuresSubfeatures[index].filter(
			f => f.index !== feature
		);
	},
	...
	selectSubclassSubtype: (state, { payload }: PayloadAction<string | null>) => {
	...
	setExpertiseProficiency: (
		...
		if (state.expertiseProficiencies.length < index + 1) {
			for (let i = 0; i <= index; ++i) {
				state.expertiseProficiencies = [...state.expertiseProficiencies, null];
			}
		}

		state.expertiseProficiencies[index] = proficiency;
	},
	addClassSpell: (state, { payload }: PayloadAction<SpellItem>) => {
		if (!state.spells) {
			state.spells = [];
		}
	...
	addSubclassSpell: (state, { payload }: PayloadAction<SpellItem>) => {
