...
export const createClassrInfoSlice = (
	overrideInitialState: DeepPartial<ClassInfoState> = {}
) =>
	createSlice({
		name: 'classInfo',
		initialState: deepMerge<ClassInfoState>(
		...
		reducers: {
			...
			addFavoredTerrain: (
				state,
				{ payload }: PayloadAction<Terrain | null>
			) => {
			...
			removeExpertiseProficiency: state => {
				...
				state.expertiseProficiencies = state.expertiseProficiencies?.slice(
					0,
					-1
				);
			},
...
const classInfoSlice = createClassrInfoSlice();
